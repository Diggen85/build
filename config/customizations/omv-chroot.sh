...
InstallOpenMediaVault() {
	...
	' /etc/rc.local
	# Special treatment for ODROID-XU4 (and later Amlogic S912 based devices). Keep all
	# NAS daemons running on the big cores and switch little cores to performance
	[ "${BOARD}" = "odroidxu4" ] && XU4_HMP_Fix='; taskset -c -p 4-7 $i ; echo performance >/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor '
